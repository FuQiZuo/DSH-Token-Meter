...
- 所有总量读取 DSH 的 `tokenUsage` projection（客户端权威值）：`输入 = uncachedInputTokens + cacheReadTokens + cacheWriteTokens`，`命中 = cacheReadTokens`，`未命中 = uncachedInputTokens + cacheWriteTokens`，`总计 = 输入 + 输出`。四个桶互斥。
